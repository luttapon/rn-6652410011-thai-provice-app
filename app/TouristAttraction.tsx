  ...
  const fetchRestaurants = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('locations')
        .select('*')
        .eq('category', 'attraction'); // ดึงเฉพาะหมวดสถานที่ท่องเที่ยว
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text>กำลังโหลดร้านอาหารอร่อยๆ...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>สถานที่ท่องเที่ยวแนะนำ</Text>
        <View style={{ width: 28 }} />
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>ไม่พบข้อมูลสถานที่ท่องเที่ยว</Text>}
      />
    </View>
  );
};
